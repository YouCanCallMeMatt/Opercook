...
void CookingProcess() {
    ...
    while (!feof(fptr)){
        fscanf(fptr, "%d#%[^#]#%d#%[^#]#%[^#]#%[^#]#%[^#]#%[^#]#%d\n", &Cooking[Index].Type, Cooking[Index].Name, &Cooking[Index].Price, Cooking[Index].Topping, Cooking[Index].Calories, Cooking[Index].Flavor, Cooking[Index].Size, Cooking[Index].Time, &Cooking[Index].Nunggu);
        Index++;
    }
    fclose(fptr);
    ...
    for (int i = 0; i < Index; i++){
        ...
        if (Cooking[i].Nunggu <= 0 && Index == 1){
            ...
            fprintf(fptr, "%d#%s#%d#%s#%s#%s#%s#%s#%d\n", Cooking[i].Type, Cooking[i].Name, Cooking[i].Price, Cooking[i].Topping, Cooking[i].Calories, Cooking[i].Flavor, Cooking[i].Size, Cooking[i].Time, Cooking[i].Nunggu);
        }
    }
    fclose(fptr2);
    ...
    if (Index >= 1){
        ...
        for (int i = 0; i < Index; i++){
            if (Cooking[i].Type == 1){
                printf("| %-4d| Dessert                  | %-25s| $%-20d| %-12ds |\n", i + 1, Cooking[i].Name, Cooking[i].Price, Cooking[i].Nunggu);
            }
